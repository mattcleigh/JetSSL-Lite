...
def get_args():
    parser = argparse.ArgumentParser(
        description="Combine all JetClass .HDF files for faster loading."
    )
    parser.add_argument(
        "--data_path",
        type=str,
        default="/srv/fast/share/rodem/JetClassH5/",
        help="The path to the JetClass train/val/test folders",
    )
    return parser.parse_args()


def main() -> None:
    """Combine all JetClass .HDF files for faster loading."""
    # Get the arguments
    args = get_args()
